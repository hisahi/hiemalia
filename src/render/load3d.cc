...
namespace hiemalia {
static ModelPoint parseModelPoint(std::string s, coord_t scale) {
    coord_t x = 0, y = 0, z = 0;
    std::sscanf(s.c_str(), FMT_coord_t " " FMT_coord_t " " FMT_coord_t, &x, &y,
                &z);
    return ModelPoint(x * scale, y * scale, z * scale);
}
...
static Model loadStream3D(std::istream& in) {
    ...
    for (std::string line; std::getline(in, line);) {
        ...
        if (command == "v") {
            vertices.push_back(parseModelPoint(value, scale));
        } else if (command == "l") {
            fragments.push_back(
...
Model load3D(const std::string& filename) {
    LOG_TRACE("loading 3D model from %s", filename);
