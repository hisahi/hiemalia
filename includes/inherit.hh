...
#define DEFAULT_COPY(T)         \
    T(const T& copy) = default; \
    T& operator=(const T& copy) = default;

#define INHERIT_COPY(T, Tbase) DEFAULT_COPY(T)
...
#define DEFAULT_MOVE(T)             \
    T(T&& move) noexcept = default; \
    T& operator=(T&& move) noexcept = default;

#define INHERIT_MOVE(T, Tbase) DEFAULT_MOVE(T)
